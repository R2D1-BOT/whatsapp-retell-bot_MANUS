...
// ============================================
// 📄 ENDPOINT PARA ENVIAR PDF - CUSTOM FUNCTION
// ============================================
app.post('/send-menu', async (req, res) => {
    try {
        console.log('📄 Custom Function llamada - Enviar PDF:', req.body);
        ...
        console.log(`📄 Enviando PDF del menú a: ${user_phone_number}`);
        ...
        console.log('✅ PDF del menú enviado correctamente');
        
        // CRÍTICO: Retell necesita respuesta 200 con JSON
        res.status(200).json({ 
            success: true,
            message: 'PDF menu sent successfully'
        });

    } catch (error) {
        console.error('❌ Error enviando PDF:', error.response?.data || error.message);
        
        // Aún en error, devolver 200 para que Retell no reintente
        res.status(200).json({ 
            success: false, 
            error: error.message 
        });
