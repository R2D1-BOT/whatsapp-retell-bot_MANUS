...
function cleanupInactiveSessions() {
  const now = Date.now();
  for (const user in sessionTimestamps) {
    if (now - sessionTimestamps[user] > INACTIVITY_TIMEOUT) {
      delete chatSessions[user];
      delete sessionTimestamps[user];
      console.log(`💤 Sesión de ${user} eliminada por inactividad`);
    }
  }
}
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    // PASAR NÚMERO A RETELL COMO VARIABLE DINÁMICA
    ...
    const lastMessage = chatCompletionResponse.data.messages.at(-1);
...
app.post('/send-menu', async (req, res) => {
  ...
  if (!senderNumber) {
    console.error('!!! ERROR: Retell no envió el user_phone_number.');
    return res.status(400).json({ error: 'Falta el número de teléfono.' });
  }

  console.log(`[Custom Function] ✅ Número recibido: ${senderNumber}. Enviando PDF...`);

  // 🔥 RESPONDER INMEDIATO A RETELL (evita timeout/SIGTERM)
  res.status(200).json({ status: 'accepted' });

  // 🚀 ENVIAR PDF EN SEGUNDO PLANO
  try {
    await axios.post(
      EVO_SEND_MEDIA_URL,
      {
        ...
        caption: "¡Aquí tienes nuestra carta! Avísame cuando sepas qué quieres pedir.",
        url: PDF_MENU_URL
      },
      { headers: { 'apikey': EVO_API_KEY } }
    );
...
// =======================================================================
// 🔥 SERVIDOR ESCUCHANDO PARA RAILWAY
// =======================================================================
